...
| `--video-decoder {conv,diffusion}` | conv | LTX 2.5 diffusion video decoder (sharper, slower; tiled automatically above `LTX2_VAE_DECODE_BUDGET_GB`). Border numerics follow upstream's default chunked mode. Refused together with `--stepwise-*` previews. On official weights it needs `ltx-2.5-video-vae-bf16.safetensors` in the download. | `generate` (all variants) |
| `--diffvae-tile FRAMES HEIGHT WIDTH` | auto | Diffusion decoder tile size in pixel frames / pixels (multiples of 2 frames / 8 px, 0 = axis untiled, `0 0 0` = force one tile under `LTX2_DIFFVAE_MAX_TOKENS`). Overlaps are fixed at 40 frames / 160 px. | with `--video-decoder diffusion` |
| `LTX2_VAE_DECODE_BUDGET_GB=N` | 8 (conv) / RAM÷2 (diffusion) | Peak-memory budget of the VAE decode; drives conv temporal tiling and diffusion decoder tile sizing. Raise on 64-256 GB machines for fewer, larger tiles. | video decode |
