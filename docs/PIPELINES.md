...
| `--video-decoder {conv,diffusion}` | conv | LTX 2.5 diffusion video decoder (sharper, slower, single tile, size-guarded by `LTX2_DIFFVAE_MAX_TOKENS`). Border numerics follow upstream's default chunked mode. | `generate` (all variants) |
